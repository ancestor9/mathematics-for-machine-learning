## 7. 회귀분석
## 8. Perceptron
## 4주차에는 2학년은 조건부확률과 히스토그램으로 34, 35, 36, 조건부확률부분은 이후 전부 다 하기

### 강의 개선사항
- 1_statistics for machine learning.pptx :  데이터 요약 및 표현 이후 부문 compact하여야 함... 조금 지루함.(3학년 수업 중)


### 가. 데이터 과학이란?
- 데이터로부터 의미 있는 분석과 통찰을 도출하고, 예측 및 의사결정을 지원하기 위해 통계학, 수학, 컴퓨터과학, 도메인 지식을 통합적으로 활용하는 학문 및 실천 분야

### 나. 데이터 과학의 분석대상(What)과 분석방법(How)에 대하여

|              |**How(Yes)**| **How(No)** |
|--------------|----------------|------------------------|
| **What(Yes)**  |  최적화 (Optimization) | 솔루션 (Solution)     |
|              |                |                        |
| **What(No)**  | 통찰 (Insight) |발견 (Discovery)  |


### 다. 데이터 과학의 역량과 스킬
[친절한 데이터 사이언티스트 되기 강좌](https://recipesds.tistory.com/)

<img src="https://machinelearningmastery.com/wp-content/uploads/2013/11/Data_Science_VD.png">

### Math & Statistics Knowledge 
[mathsisfun](https://www.mathsisfun.com/)

[same-stats-different-graphs](https://www.research.autodesk.com/publications/same-stats-different-graphs/)
1. Probability & Statistics
2. Derative, Modeling & Simulation

### Hacking Skills (Hard skills)
1. Python, Javascripts, HTML/CSS, React
2. FastAPI, Webcrwaling, pandas, numpy, DataBase
3. pytorch, tensorflow, Keras
4. Generative AI programing (Langchain, MCP, AI agent)

### Subtantive Expertise
1. Domain Knowledge
2. Industry Experience
